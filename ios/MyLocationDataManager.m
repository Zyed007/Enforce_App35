...
//                           };
//
//     [self sendEventWithName:@"significantLocationChange"
//     body:lastLocationEvent];
// }
//
//@end
//
...
RCT_EXPORT_METHOD(initialize : (RCTPromiseResolveBlock)
...
RCT_EXPORT_METHOD(hasPermissions : (NSString *)permissionType
                      hasPermissionsWithResolver : (RCTPromiseResolveBlock)
                          resolve rejecter : (RCTPromiseRejectBlock)reject) {
  RCTLogInfo(@"Pretending to do something natively: hasPermissions %@",
             permissionType);
...
RCT_EXPORT_METHOD(requestPermissions : (NSString *)permissionType
                      requestPermissionsWithResolver : (RCTPromiseResolveBlock)
                          resolve rejecter : (RCTPromiseRejectBlock)reject) {
  NSArray *arbitraryReturnVal = @[ @"testing..." ];
  RCTLogInfo(@"Pretending to do something natively: requestPermissions %@",
             permissionType);
...
RCT_EXPORT_METHOD(stop) { [locationManager stopUpdatingLocation]; }
...
- (void)locationManager:(CLLocationManager *)manager
  ...
  // TODO: do something meaningful with our location event. We can do that here,
  // or emit back to React Native
